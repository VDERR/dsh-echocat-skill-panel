...
> 4.4.0 液态银蓝本地开发版：新增丰富的本机背景自定义，保留连续表面、卡片层级和原业务流程。详见 [4.4.0 更新说明](RELEASE_NOTES_4.4.0.md)。本版完成离线真实组件验证，尚未进行 DSH 实机安装验收。

**v4.4.0** · DSH Desktop Beta 的 cordis 插件：统计每轮对话用到了哪些 skill，并让你**在应用里直接装卸、更新它们**。
