...
- **467 条具名调整**：215 项行为打磨（[polish.js](src/client/polish.js)）+ 252 项设计重做（[design.js](src/client/design.js)），全部是带名字和理由的数据记录，样式表由它们生成，测试逐条核对真的生效。
...
架构与实现要点、安装引擎的四条安全规矩、**宽度为什么用百分比而不是像素**、**停用为什么是"移出目录"而不是改名**、**467 条调整为什么是生成的**、**设计重做是怎么"先看见再改"的**、**来源记录为什么放在 skill 目录里**、**踩过的坑**（宿主语义令牌不能当视觉令牌用、逗号选择器不能共享组合符、`min-height:0` 会架空粘底页脚……），以及 1290 项断言 / 十一个门的测试明细 —— 见 [docs/设计要点.md](docs/设计要点.md)。
