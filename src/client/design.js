...
const DESIGN = Object.freeze([
  /* ============================ 1. tokens ============================ */
  //
  // The palette moved from COOL NEUTRAL to WARM SOFT, and the reason is worth writing down
  // because it is the difference the user could actually see.
  //
  // The old greys were blue-tinted (#f7f7f8 / #fbfbfc / #f2f2f4) and sat within 1–2% of each
  // other, so the panel read as one flat field of "computer grey" and every card had to lean on
  // a hairline border to be visible at all — which is exactly what "全是细线小盒子" was
  // describing. The warm scale below (from the Scandinavian direction: #faf8f5 canvas, muted
  // naturals) separates by TEMPERATURE as well as by luminance, and the steps are 4–7% apart
  // instead of 1–2%, so a card is legible as a card without a border doing the work.
  //
  // Contrast was re-checked after the change, not assumed: the assertions in
  // test/ui-polish.mjs compute WCAG ratios from these exact values and fail if one drops
  // below AA.
  D('color-canvas', 'token', '{root}', { '--sr-canvas': '#faf8f5' },
    'warm off-white: it separates from a white card by temperature, not only by brightness'),
  D('color-card', 'token', '{root}', { '--sr-card': '#ffffff' },
    'the panel surface itself, one deliberate step above the canvas'),
  D('color-raised', 'token', '{root}', { '--sr-raised': '#f6f4f1' },
    'cards and inputs: a warm step that is 4% from white, so a card reads as a card'),
  D('color-sunken', 'token', '{root}', { '--sr-sunken': '#f0ede8' },
    'tracks, wells and code: the only place that goes BELOW the surface, and visibly so'),
  D('color-fill', 'token', '{root}', { '--sr-fill': 'rgba(28,25,23,.05)' },
    'hover fills are warm ink rather than a neutral grey with its own blue cast'),
  D('color-fill2', 'token', '{root}', { '--sr-fill2': 'rgba(28,25,23,.03)' },
    'the faintest wash, for large quiet areas'),
  D('color-line', 'token', '{root}', { '--sr-line': 'color-mix(in srgb, #1c1917 9%, transparent)' },
    'hairline derived from the ink colour, so it is the right weight on every surface'),
  D('color-line2', 'token', '{root}', { '--sr-line2': 'color-mix(in srgb, #1c1917 17%, transparent)' },
    'the stronger hairline, for hover edges and control borders'),
  D('color-ink', 'token', '{root}', { '--sr-fg': '#1c1917' },
    'warm near-black: it belongs to the canvas, where a blue-black fought it'),
  D('color-ink3', 'token', '{root}', { '--sr-fg3': '#6e6a68' },
    'tertiary text clears AA on the SUNKEN step as well as white (4.58:1 worst). #7c7773 was the warm value I wanted and it failed at 3.79:1 — the suite caught it, so this is the lightest warm grey that passes on all three surfaces it is used on'),
  D('color-ink2', 'token', '{root}', { '--sr-fg2': '#57534e' },
    'secondary text, readable at 11.5px rather than merely present (7.1:1 on white)'),
  D('color-accent', 'token', '{root}', { '--sr-accent': '#5b5bd6' },
    'a softer indigo: the old #4f46e5 was the last cold colour in a warm panel and it shouted against it'),
  ...
  D('color-danger', 'token', '{root}', { '--sr-danger': '#c2410c' },
    'a warm brick rather than a signal red: destructive actions here are available, not urgent'),
  ...
  D('wash-root', 'frame', '.sr-root', {
    backgroundImage: 'linear-gradient(180deg, color-mix(in srgb, var(--sr-accent) 3%, transparent), transparent 220px)',
    backgroundRepeat: 'no-repeat',
  }, 'a faint cool wash at the top of the panel, so the header sits on something rather than on white'),
