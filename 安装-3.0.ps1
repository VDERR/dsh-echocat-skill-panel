...
.EXAMPLE
  & ".\安装-3.0.ps1"
  & ".\安装-3.0.ps1" -SkipInstall          # manifest + files only
  & ".\安装-3.0.ps1" -Rollback             # undo the last install
#>
[CmdletBinding()]
param(
  ...
  [string[]]$OldNames = @('echocat-skill-panel-2.0', 'EchoCat-skill-Panel-2.0', 'dsh-skill-report'),
  [switch]$SkipInstall,
  [switch]$Rollback
)
