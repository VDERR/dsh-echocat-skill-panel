...
check('the logo switch defaults to visible',bg.DEFAULTS.logo==='show'&&bg.normalize(null).logo==='show')
check('the logo switch accepts both of its two states',bg.normalize({logo:'hide'}).logo==='hide'&&bg.normalize({logo:'show'}).logo==='show')
check('an unknown logo value falls back to visible',bg.normalize({logo:'none'}).logo==='show')
check('a coercible truthy logo value is not mistaken for a state',bg.normalize({logo:true}).logo==='show'&&bg.normalize({logo:0}).logo==='show')
